...
class MultiPatternAnalyzer:
    ...
    def analyze(self, data):
        ...
        print(
            f"Time range: {datetime.fromtimestamp(data.timestamps[0])} to {datetime.fromtimestamp(data.timestamps[-1])}"
        )
        ...
        # Determine consensus
        if len(buy_signals) > len(sell_signals) and len(buy_signals) > len(
            hold_signals
        ):
            consensus = "BUY"
            confidence = len(buy_signals) / len(all_patterns) if all_patterns else 0
            agreement = len(buy_signals)
        elif len(sell_signals) > len(buy_signals) and len(sell_signals) > len(
            hold_signals
        ):
            consensus = "SELL"
        ...
        print(f"\n→ CONSENSUS: {consensus}")
        print(
            f"  Agreement: {agreement}/{len(all_patterns)} patterns ({confidence*100:.1f}%)"
        )
...
def main():
    ...
    for name, result in test_cases:
        print(
            f"{name:<20} {result['consensus']:<10} {result['confidence']:.2f}         {result['total_patterns']}"
        )
    ...
    for name, _ in analyzer.all_detectors:
        total_detected = sum(
            len(r["patterns_by_detector"].get(name, [])) for _, r in test_cases
        )
