...
async def main():
    ...
    try:
        ...
        print(
            f"  Date range: {datetime.fromtimestamp(data.timestamps[0]).date()} to "
        ...
        # Technical patterns
        tech_patterns = await technical_detector.detect_patterns(
            symbol, timeframe, data
        )
        all_patterns.extend(tech_patterns)

        # Candlestick patterns
        candle_patterns = await candlestick_detector.detect_patterns(
            symbol, timeframe, data
        )
        ...
        for strategy in strategies:
            ...
            if results["trades"]:
                ...
                for i, trade in enumerate(results["trades"][:5]):  # Show first 5
                    ...
                    if trade.pattern:
                        print(
                            f"    Pattern: {trade.pattern.pattern_name} (confidence: {trade.pattern.confidence:.2%})"
                        )
        ...
        print(
            f"{'Strategy':<30} {'Return':<12} {'Sharpe':<10} {'Max DD':<12} {'Win Rate':<12} {'Trades'}"
        )
        print("-" * 95)
        for s in comparison:
            print(
                f"{s['name']:<30} {s['return']:>10.2f}%  {s['sharpe']:>8.2f}  {s['max_dd']:>10.2f}%  "
