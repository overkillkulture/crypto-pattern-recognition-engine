...
from src.utils.metrics import (calculate_atr, calculate_bollinger_bands,
                               calculate_macd, calculate_obv, calculate_rsi,
...
class RSIPattern(Pattern):
        ...
        if current_rsi > self.overbought:
            results.append(
                PatternResult(
                    ...
                    confidence=min(
                        (current_rsi - self.overbought) / (100 - self.overbought), 0.99
                    ),
    ...
    def validate(self, result: PatternResult) -> bool:
        ...
        return (
            result.pattern_type == PatternType.TECHNICAL_INDICATOR
            and "rsi" in result.metadata
            and 0 <= result.metadata["rsi"] <= 100
        )
...
class MACDPattern(Pattern):
    ...
    def validate(self, result: PatternResult) -> bool:
        ...
        return (
            result.pattern_type == PatternType.TECHNICAL_INDICATOR
            and "macd" in result.metadata
            and "signal" in result.metadata
            and "histogram" in result.metadata
        )
...
class ADXPattern(Pattern):
    ...
    def detect(self, data: OHLCV) -> List[PatternResult]:
        ...
        if (
            ...
            and prev_adx <= self.strong_trend_threshold
        ):
